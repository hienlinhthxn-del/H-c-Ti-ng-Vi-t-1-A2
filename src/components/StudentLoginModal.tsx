...
export const StudentLoginModal: React.FC<StudentLoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const handleVerifyCode = (e: React.FormEvent) => {
    ...
    if (classCode.trim().toUpperCase() === '1A2') {
      ...
      setError('Mã l?p không chính xác. Hãy h?i Cô giáo nhé!');
      ...
      <div className="bg-white rounded-[32px] w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-blue-500 p-6 text-white text-center relative shrink-0">
          ...
          <p className="text-blue-100 font-medium mt-1">
            {!isCodeValid ? 'Vui lòng nh?p Mã l?p d? vào l?p h?c' : 'Tìm và b?m vào tên c?a em nhé!'}
          </p>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto">
          {!isCodeValid ? (
            <form onSubmit={handleVerifyCode} className="space-y-6 py-8">
              <div>
                <label className="block text-center text-slate-700 font-bold mb-4 text-lg">Mã l?p c?a em là gì?</label>
                <input
                  type="text"
                  value={classCode}
                  onChange={(e) => setClassCode(e.target.value)}
                  placeholder="Nh?p mã l?p (VD: 1A2)"
            ...
            <div className="space-y-4">
              <h3 className="text-center font-bold text-blue-800 text-lg">Danh sách L?p 1A2 ({students.length} h?c sinh)</h3>
